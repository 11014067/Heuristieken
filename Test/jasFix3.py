...
class Battery:
	...
	def add_house(self, house):
		...
		if self.spare_voltage > house.voltage:
			if self.min_x > house.x:
				self.min_x = house.x
				self.houses_list.append(house.name)
	...
	def remove_house(self, house):
		self.houses_list.remove(house.name)
		self.spare_voltage += house.voltage
...
for house in houses:
	...
	while unplaced:
		...
		if i > 4 and unplaced == True:
			misplaced.append(house.name)
			unplaced = False
	housenumber+=1

print batteries[0].houses_list
...
for i in range(0,len(batteries)):
	...
	if batteries[i].spare_voltage > maxVolt:
		maxVolt2 = maxVolt
		maxIndex2 = maxIndex
		maxVolt = batteries[i].spare_voltage
		maxIndex = i
	elif batteries[i].spare_voltage > maxVolt2:
		maxVolt2 = batteries[i].spare_voltage
		maxIndex2 = i
...
maxHouse = 0
iMaxHouse = 0
for index in batteries[maxIndex].houses_list:
	if maxHouse < houses[index].voltage:
		maxHouse = houses[index].voltage
		iMaxHouse = index


minHouse = 99
iMinHouse = 0
for index in batteries[maxIndex2].houses_list:
	if minHouse > houses[index].voltage:
		minHouse = houses[index].voltage
		iMinHouse = index

print batteries[maxIndex].houses_list

batteries[maxIndex].remove_house(houses[iMaxHouse])
batteries[maxIndex2].remove_house(houses[iMinHouse])
batteries[maxIndex].add_house(houses[iMinHouse])
batteries[maxIndex2].add_house(houses[iMaxHouse])

if batteries[maxIndex].add_house(houses[misplaced[0]]):
	print "GELUKT!!!"
